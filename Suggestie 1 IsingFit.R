...
library(qgraph)
IsingFit2 <-
  function(x, family = "binomial", AND = TRUE, gamma = 0.25, plot = TRUE, progressbar = TRUE, sum.split = 0, lowerbound.lambda = NA, ...) {
    t0 <- Sys.time()
    xx <- x
    if (family != "binomial") {
    ...
    N <- vector()
    for (i in 1:nvar) {
      subData <- x[rowSums(replace(x[, -i, drop = FALSE], x[, -i, drop = FALSE] < 0, 0)) != (sum.split - 1), ]
...
plot.IsingFit2 <- function(object,...) qgraph(object$q,DoNotPlot = FALSE, ...)

print.IsingFit2 <- function(x)
{
...
summary.IsingFit2 <- function(object)
{
