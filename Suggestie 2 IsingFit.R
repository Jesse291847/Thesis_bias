library(glmnet)
library(qgraph)
IsingFit3 <- function(x, family='binomial', AND = TRUE, gamma = 0.25, plot = TRUE, progressbar = TRUE, sum.split = FALSE,
    ...
    return(Res)
      ...
      N <- vector()
      for (i in 1:nvar) {
        subData <- x[rowSums(x[, -i, drop = FALSE]) != (sum.split - 1), ]
      ...
      stop("Invalid sum score supplied: should be a number smaller than the amount of variables in the data")
    }
  }
